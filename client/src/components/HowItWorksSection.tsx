    ...
    <section ref={ref} className="relative py-24 overflow-hidden">
      ...
      <div className="relative z-10 container max-w-5xl">
        ...
        >
          ...
          <span className="font-[var(--font-mono)] text-xs text-cyan-600 uppercase tracking-widest">
            How It Works
          </span>
        </div>

        <h2
          className={`text-3xl sm:text-4xl lg:text-5xl font-bold text-slate-900 mb-4 transition-all duration-700 ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
        >
          From Upload to Leverage
          <br />
          <span className="text-cyan-600">in Under 60 Seconds.</span>
        </h2>

        <p
          className={`text-slate-500 text-lg mb-16 max-w-xl transition-all duration-700 delay-200 ${isInView ? "opacity-100" : "opacity-0"}`}
        >
        ...
        <div className="relative">
          {/* Connecting line */}
          <div
            className="absolute left-[27px] top-0 bottom-0 w-[1px] hidden md:block"
            style={{
              background:
                "linear-gradient(180deg, rgba(8,145,178,0.3), rgba(16,185,129,0.3))",
            }}
          />
              ...
              return (
                ...
                >
                  ...
                  <div className="relative flex-shrink-0">
                    <div
                      className={`w-14 h-14 rounded-xl flex items-center justify-center ${cls.iconBg} border ${cls.iconBorder}`}
                    >
                      <step.icon className={`w-6 h-6 ${cls.iconColor}`} />
                    </div>
                  </div>

                  {/* Content */}
                  <div className="bg-white/80 backdrop-blur-xl shadow-lg border border-slate-200 rounded-2xl p-6 flex-1">
