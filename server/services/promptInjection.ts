...
export function detectPromptInjection(ocrText: string): PromptInjectionDetection {
  ...
  const hits: { sig: InjectionSignal; weight: number; re: RegExp }[] = [
    ...
    { sig: "tooling_override", weight: 20, re: /\b(open|use)\s+(tools|browser|terminal|database|server logs)\b/gi },
    { sig: "data_exfiltration", weight: 25, re: /\b(api key|secret|token|credentials|session|cookie)\b/gi },
    { sig: "data_exfiltration", weight: 20, re: /\bexfiltrate\b|\bleak\b|\bprint\b.*\b(full|raw)\b/gi },
